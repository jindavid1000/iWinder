...
void Protocol::sendStatus() {
    ...
    doc["length"]             = roundf(g_state.lengthTheoretical * 100) / 100.0;
...
void Protocol::cmdCalibrateServo() {
    if (g_state.state != STATE_IDLE) {
        sendResponse("calibrate_servo", false, "设备需处于待机状态");
        return;
    }
    g_winder.startServoCalib();
    sendResponse("calibrate_servo", true, "舵机标定开始");
}
