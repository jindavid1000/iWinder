...
![Status](https://img.shields.io/badge/status-v0.2.10%20stable-brightgreen)
...
| 电机驱动（二选一） | ① IRLB8721 MOS + 150Ω/10kΩ 电阻 + 1N5822（PWM 调速）② L298N 模块（开关，不调速） | ①不可用 IRF520（栅极需 10V）；②接线简单：ENA 插跳线、IN1=GPIO4、IN2=GND |
...
| 5V | 舵机 VCC |
| 12V | 电机供电：MOS 方案——电机正极，电机两端并联 1N5822 续流二极管（负极朝 12V 端），电机负极接 IRLB8721 漏极（D），源极（S）接 GND，栅极经 150Ω 接 GPIO4、10kΩ 下拉到 GND；L298N 方案——接模块 +12V 输入 |
| GND | 所有模块共地 |
...
> 磁场质量诊断和匀速偏心检测，并在每趟小结限位间圈数 × 导程（⚠️ 含停稳回弹约 5mm，
> 精确限位间距以定位日志的右锚定坐标 − 左触发坐标为准）。
...
1. **选驱动模式**：设置页「驱动模式」→ 电动（电机驱动）或 手动（手摇驱动，电机不输出）。
   用 L298N 驱动板的话把「电机驱动电路」切到 L298N——此模式无调速，电动模式启动即全速
...
自动触发一次限位校准并回到原位置继续。电动模式带缠料保护（电机运转但料盘停转 4 秒
自动**暂停**并提示，清理后点「恢复」继续绕线，无需重新定位）。
...
| 电机驱动电路 | MOS 管调速（PWM，支持软启动/调速滑块）/ L298N 开关（ENA 插跳线全速，只能启停不能调速，方向由接线决定） |
| 位置反馈 | 舵机开环估算 / AS5600 编码器闭环（装编码器后强烈建议开启；绕线中为连续旋转速度自适应控制，平滑跟随） |
| 排线速度标定 | 自动测量满速 3 个来回 + 步进速度 1 个来回；编码器模式下还会标定每圈位移 |
| 限位间距 | 左右限位**触发点**之间的真实距离（默认 62mm）。⚠️ 用 `test_encoder` 测量时注意「右停稳→左触发」含柔性回弹（偏大约 5mm），最准的方法是：绕线/定位日志中右锚定坐标 − 左触发坐标 |
| 霍尔去抖 | 默认 25000us（滤除电机 PWM 耦合噪声），真实转速超 300RPM 才需调小 |
| 料盘参数 | 外径、宽度（拓竹料盘宽 60mm，默认已填）、芯轴直径（有/无纸筒可切换）——只影响长度/层数计算 |
| 绕线范围 | **右基准模型**：右终止位置（默认 62mm，由机器安装决定）+ 料盘宽度 → 左起始自动推导（= 右终止 − 宽度）。换盘只改宽度一个数字；料盘左右都不固定的特殊安装可勾「左起始手动指定」。绕线宽度 = 右终止 − 左起始，界面实时校验与料盘宽度的一致性 |
| 校准间隔 | 电动: 每几个来回校准一次；手动: 停转触发校准前至少需要的来回数 |
| 缠料保护 | 电动模式电机运转但料盘 0 转速持续 4 秒自动**暂停**（保留进度），清理后点「恢复」继续 |
...
设备连上 WiFi 后，在 Web 界面或 APP 的「授权」页填个昵称，点**「一键申请」**
即可自动获取并填入许可证（自动批准），点**激活**就能用了。也可以到
[授权网站](https://tt.jyking.dpdns.org:1000/) 网页申请：填设备 ID 提交，
把返回的许可证粘贴到设备「授权」页激活。

**方式二：QQ 群**

加入 QQ 群 **1103884695** 找作者，发设备 ID（授权页可查）即可。

> 未授权时可以正常连接、调参、回原点、标定，只是无法启动绕线任务。
> 许可证默认有效期一年，到期在原页面重新申请即可（免费，自动批准）。
> 商业授权（整机/套件销售、衍生产品）通过 QQ 群联系。
